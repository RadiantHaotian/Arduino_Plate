...
void Adafruit_TFTLCD::init(void) {
  ...
  RD_IDLE;
  CD_DATA;
  digitalWrite(A15, HIGH); // Reset line
  pinMode(A14, OUTPUT);   // Enable outputs
  pinMode(A13, OUTPUT);
  pinMode(A12, OUTPUT);
  pinMode(A11, OUTPUT);
  pinMode( A15, OUTPUT);
...
void Adafruit_TFTLCD::begin(uint16_t id) {
 ...
 void Adafruit_TFTLCD::LCD_sleep(uint16_t flag)
 {
	   if(flag)
	   {
		    CD_COMMAND; 
		  ...
		  //  writeRegister8(0x00,0x28);
			delay(120);
			CD_COMMAND; 
			write8(0x00);
			CD_DATA;
			write8(0x10); 
			//writeRegister8(0x00,0x10);
	   }
	   else 
	   {
		   writeRegister8(0x00,0x11);
			delay(120);
			writeRegister8(0x00,0x29);
	   }
 }
